...
static KEY_OBJ g_KeyTable[] =
{
    ...
    {FLGKEY_MODE,         KEY_CONTINUE,       NVTEVT_KEY_MODE,         NVTEVT_KEY_CONTINUE,       0},
...
#define KEYSCAN_POWEROFF_COUNT  150 //20ms is 1
#else
...
#if 1
    {
        ...
        // Debounce
            if (uiPowerKey == KEYSCAN_PWRKEY_RELEASED)
            {
                ...
                #if(((_MODEL_DSC_==_MODEL_DUAL_D333_) && (TEST_FUNCTION==ENABLE))\
                    || (_MODEL_DSC_==_MODEL_CARDV_K12_)\
                    || (_MODEL_DSC_==_MODEL_DUAL_AONI328_))//vincent@20150825-3
                    if((KeyScan_GetPlugDev() != 1)&&(KeyScan_GetPlugDev() != 2))
                    {
                        debug_msg("vincent@20150825 power detect:short press!!!!\r\n");
                      	UISound_Play(DEMOSOUND_SOUND_KEY_TONE);	
...
#elif (_MODEL_DSC_ == _MODEL_CARDV_K12_)//vincent@20150825-2
#define KEY_ADC_MODE         (692)
#else
#define KEY_ADC_MODE         (514)
#endif
...
#define CUSTOME_MODE_KEY      (1)

#if(_MODEL_DSC_== _MODEL_DUAL_NAZHIDA_)
#define CUSTOME_MENU_KEY      (0)
#define CUSTOME_UP_KEY        (1)
#define CUSTOME_DOWN_KEY      (1)
#elif(_MODEL_DSC_ == _MODEL_DUAL_AONI328_)
#define CUSTOME_MENU_KEY      (1)
#define KEY_ADC_MENU          (0)
#define KEYSCAN_MENULONGPRESS_COUNT (150)

#define CUSTOME_UP_KEY        (0)
#define CUSTOME_DOWN_KEY      (0)
#endif
...
void UI_DetCustomerModeKey(void)
{
    ...
    static UINT32 uiMenuKey    = KEYSCAN_CUSTOMER_UNKNOWN;
	static UINT32 uiMenuState    = KEYSCAN_CUSTOMER_INIT_STATE;
        ...
        if (uiModeKey == KEYSCAN_CUSTOMER_PRESSED)
        {
            ...
            if(uiModeState == KEYSCAN_CUSTOMER_PRESS_STATE)
            {
		     ...
		     uiModeState    = KEYSCAN_CUSTOMER_INIT_STATE;			
        		...
        		{		
                    #if((_MODEL_DSC_== _MODEL_CARDV_K12_))//vincent@20150825-2
                        debug_msg("vincent@20150825 UI_DetCustomerModeKey:mode key long press!!\r\n");
	...
	else
	{
        ...
        if (uiModeKey == KEYSCAN_CUSTOMER_RELEASED)
        {
            ...
            {	   
        		...
        		{
                    #if((_MODEL_DSC_== _MODEL_CARDV_K12_))//vincent@20150825-2
                    debug_msg("vincent@20150825 UI_DetCustomerModeKey:mode key short press!!\r\n");
                    Ux_PostEvent(NVTEVT_KEY_MODE, 1, NVTEVT_KEY_PRESS);	
                    #else
                    Ux_PostEvent(NVTEVT_KEY_MENU, 1, NVTEVT_KEY_PRESS);	//should post NVTEVT_KEY_MODE here?
                    #endif
...
#if CUSTOME_MENU_KEY
    if((uiADCValue>=(KEY_ADC_MENU-KEY_ADC_RANGE))&&(uiADCValue<(KEY_ADC_MENU+KEY_ADC_RANGE)))
	{
        // Debounce
        if (uiMenuKey == KEYSCAN_CUSTOMER_PRESSED)
        {
            if (uiMenuState == KEYSCAN_CUSTOMER_RELEASE_STATE)
            {
                uiMenuKeyPressCount++;
                debug_err(("uMenuKeyPressCount : %d....%d\r\n",uiMenuKeyPressCount,KEYSCAN_CUSTOMER_COUNT));
				
                if (uiMenuKeyPressCount > KEYSCAN_MENULONGPRESS_COUNT)
                {
                    uiMenuState = KEYSCAN_CUSTOMER_PRESS_STATE;
                }
            }

            if(uiMenuState == KEYSCAN_CUSTOMER_PRESS_STATE)
            {
                  //uiKeyCode = FLGKEY_KEY_POWER;
    		     uiMenuKeyPressCount = 0;		
    		     uiMenuKey    = KEYSCAN_CUSTOMER_UNKNOWN;
    		     uiMenuState    = KEYSCAN_CUSTOMER_INIT_STATE;
        		...
        		{		
                    Ux_PostEvent(NVTEVT_KEY_MENU, 1, NVTEVT_KEY_CONTINUE); 
        		}
        ...
        {
            uiMenuKey = KEYSCAN_PWRKEY_PRESSED;
        }
    }
	else
	{
        // Debounce
        if (uiMenuKey == KEYSCAN_CUSTOMER_RELEASED)
        {
            if (uiMenuState == KEYSCAN_CUSTOMER_INIT_STATE)
            {
                uiMenuState = KEYSCAN_CUSTOMER_RELEASE_STATE;
            }
        ...
        {
            uiMenuKey = KEYSCAN_CUSTOMER_RELEASED;
        }
	}
#endif
...
#elif((_MODEL_DSC_ == _MODEL_CARDV_CHINA_ELECTRONICS_)\
    ||(_MODEL_DSC_ == _MODEL_CARDV_K12_)\
    ||(_MODEL_DSC_ == _MODEL_DUAL_AONI328_))
        UI_DetCustomerModeKey();			
#elif((_MODEL_DSC_== _MODEL_DUAL_G2800_)||(_MODEL_DSC_ == _MODEL_DUAL_QIZHENG_M1_))
        UI_DetCustomerModeKey();		
#elif(_MODEL_DSC_== _MODEL_DUAL_NAZHIDA_)
