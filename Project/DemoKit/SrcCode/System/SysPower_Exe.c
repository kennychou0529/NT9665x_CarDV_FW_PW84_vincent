...
#if(_MODEL_DSC_ == _MODEL_CARDV_K12_)//vincent@20150805-1
#define GSENSOR_STATIC_COUNTER (9000)// 3 mins
#define GSENSOR_MOTION_COUNTER (1)
#define GSENSOR_MOTION_CONTINUE_MINTIME (400000) //in us
#define GSENSOR_MOTION_CONTINUE_MAXTIME (2000000) //in us
#endif
...
BOOL g_bLapseRecFlag = TRUE;//vincent@20150805-1
...
#elif(_MODEL_DSC_==_MODEL_CARDV_K12_)//vincent@20150814-1
SX_TIMER_ITEM(GxPower_DetBattery, UI_DetBattery, 50, TRUE)
#else
    ...
    if(!UI_IsForceLock())
    {
        ...
        // send NVTEVT_BATTERY event to flash power charging icon in power charging mode
         ...
         if (GxUSB_GetConnectType() == USB_CONNECT_CHARGER)
         {
            ...
            GxPower_DetBattery();
            Ux_PostEvent(NVTEVT_BATTERY, 0);//vincent@20150921-1
         }
        #else
        GxPower_DetBattery();
        #endif
...
void UI_DetGsensor(void)
{
    ...
    static BOOL bNormalFlag = TRUE;
    static UINT32 uLastTickCount = 0;
    UINT32 uTickDiff = 0;
		        ...
		        if(UI_GetData(FL_MOVIE_GSENSOR)!=GSENSOR_OFF)
			  {
                ...
                if (SysGetFlag(FL_MOVIE_PARKING) == MOVIE_PARKING_ON)
                {
                    ...
                    if (g_bLapseRecFlag)
                    {
                        motionCounter++;
    		            staticCounter = 0;
                        //debug_msg("vincent@20150814 gsensor shift!!!!!!\r\n");
                        uCurTickCount = Perf_GetCurrent();
                        uTickDiff = abs(uLastTickCount - uCurTickCount);
                        uLastTickCount = uCurTickCount;
                        //if (((uTickDiff >= GSENSOR_MOTION_CONTINUE_MINTIME)&&(uTickDiff < GSENSOR_MOTION_CONTINUE_MAXTIME))
                        //    && (!bNormalFlag)
                         //   && (gMovData.TimerCount >= 3))//vincent@20150908-1
    		            if ((motionCounter >= GSENSOR_MOTION_COUNTER) && (!bNormalFlag))//start normal recording
    		            {
                    ...
                    else
                    {
                        staticCounter++;
        	            motionCounter = 0;
                        //debug_msg("vincent@20150814 gsensor not shift!!!!!!\r\n");
        	            if ((staticCounter > GSENSOR_STATIC_COUNTER) && (bNormalFlag))//start time lapse recording
        	            {
                            //debug_msg("vincent@20150814 gsensor not shift:Ux_PostEvent(NVTEVT_RECORDING_MODE_TRIG, 1, 1)\r\n");
        	                Ux_PostEvent(NVTEVT_RECORDING_MODE_TRIG, 1, 1);
                            
        	                nLapseRecLedOffCounter = 30;//vincent@20150919-6
        	                bNormalFlag = FALSE;
        	            } 
                    }
                }
