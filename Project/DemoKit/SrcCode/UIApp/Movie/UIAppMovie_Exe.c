...
void Movie_SetRecParam(void)
{
    ...
    #if (UI_STYLE==UI_STYLE_DRIVER)
    #if ((_MODEL_DSC_ == _MODEL_DUAL_V18_DT_)||(_MODEL_DSC_ == _MODEL_DUAL_V18_LYSZ_)||(_MODEL_DSC_ == _MODEL_DUAL_AONI328_))//vincent@20150915-3
...
INT32 MovieExe_OnOpen(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    static BOOL bFirstSetDualRecFlag = FALSE;
...
UINT32 Movie_GetCyclicRecTime(void)
{
    ...
    default:
        uiCyclicRecTime = 300;
        break;
#elif (_MODEL_DSC_==_MODEL_DUAL_D333_)//vincent@20150824-1
    case MOVIE_CYCLICREC_1MIN:
        uiCyclicRecTime = 60;
        break;
...
INT32 MovieExe_OnRecStart(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    strcpy(gpsdata.IQBuildDate,IQS_GetBuildDate());
        #if(_MODEL_DSC_==_MODEL_DUAL_D333_)//vincent@20150902-5
...
//vincent@20150804-1 begin
        ...
        switch (SysGetFlag(FL_DATE_FORMAT))
        {
            case DATE_FORMAT_DMY:
                uiFlag |= STAMP_DD_MM_YY;
                break;
            case DATE_FORMAT_MDY:
                uiFlag |= STAMP_MM_DD_YY;
                break;
            case DATE_FORMAT_YMD:
            default:
                uiFlag |= STAMP_YY_MM_DD;
                break;
...
//vincent@20150804-1 end
        if (SysGetFlag(FL_MOVIE_DUAL_REC))
        {
            ...
            MovieStamp_Setup(
                ...
                STAMP_DATE_TIME |
                //STAMP_BOTTOM_LEFT|
                STAMP_BOTTOM_RIGHT|//vincent@20150902-4
                STAMP_POS_NORMAL |
                STAMP_BG_TRANSPARENT |
                STAMP_YY_MM_DD |
#if  (_SENSORLIB2_ != _SENSORLIB2_DUMMY_)
