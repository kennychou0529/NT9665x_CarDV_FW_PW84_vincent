...
#define SYSPARAM_LOGO_LEN    (0X5DC0)//(0X20000)//jpg size:28k
...
char g_szSecID[64] = {'O','N','L','O','G','O'};

/*
char *PsBGLogoIDIndex[]={
	"ONLOGO1",
	"ONLOGO2",
	"ONLOGO3",
	"ONLOGO4",
...
BOOL UI_ShowLogoFile(char* secId,UINT32 *uiJpgAddr,UINT32 *uiJpgSize)
{
    UINT32      uiFileSize;
    UINT8       *BgBuf;
    ER          erReturn;
...
static BOOL UI_WriteLogoFileName(char* path,char *psId)
{
    FST_FILE    pFile;
    UINT32      uiFileSize;
    UINT8       *BgBuf;
    ...
    FileSys_WaitFinish();
    pFile = FileSys_OpenFile(path, FST_OPEN_READ);
    ...
    uiFileSize = FileSys_GetFileLen(path);  // for small page nand
    ...
    *(BgBuf+3) = HI_BYTE(HI_WORD(uiFileSize));
    ...
    pSecHdl = PStore_OpenSection(psId, PS_RDWR | PS_CREATE);
...
void UI_UpdateLogoFile(void)
{
    ...
    BOOL      ret, ret2;
    char szPicFile[64] = {0};
    char szSectionID[64] = {0};
    char szLogo[64] = {0};
    int index = 0;

    
    ER formatRst = PStore_Format(&format);
    Delay_DelayMs(200);
    ...
    ret  = UI_WriteLogoFileName("A:\\logo.jpg", PS_POWERON_LOGO);
    ret2 = UI_WriteLogoFileName("A:\\logo2.jpg", PS_POWEROFF_LOGO);
...
#else
    for (index=1; index<=POWER_ON_ANIMATION_PIC_NUMBER; index++)
    {
        sprintf(szPicFile, "A:\\z1a_%d.jpg",index);
        sprintf(szSectionID, "%s%d", g_szSecID, index);
        //sprintf(szSectionID, "ONLOGO%d", index);

        ret  = UI_WriteLogoFileName(szPicFile, szSectionID);

        if (ret == TRUE)
        {
            Cal_FillRect(&Rect, _OSD_INDEX_BLACK);
            sprintf(szLogo, " Update logo %s  OK!\n", &szPicFile[3]);
            Cal_ShowStringByColor(szLogo, &Rect, _OSD_INDEX_YELLOW, _OSD_INDEX_BLACK);
            Delay_DelayMs(500);
        }
        else
        {
            Cal_FillRect(&Rect, _OSD_INDEX_BLACK);
            sprintf(szLogo, " Update logo %s  failed!\n", &szPicFile[3]);
            Cal_ShowStringByColor(szLogo, &Rect, _OSD_INDEX_YELLOW, _OSD_INDEX_BLACK);
            Delay_DelayMs(500);
        }
    }

    ret = UI_WriteLogoFileName("A:\\logo2.jpg", PS_POWEROFF_LOGO);
    if (ret == TRUE)
    {
        Cal_FillRect(&Rect, _OSD_INDEX_BLACK);
        Cal_ShowStringByColor(" Update logo off OK!\n", &Rect, _OSD_INDEX_YELLOW, _OSD_INDEX_BLACK);
        Delay_DelayMs(500);
    }
    else
    {
        Cal_FillRect(&Rect, _OSD_INDEX_BLACK);
        Cal_ShowStringByColor(" Update logo off failed!\n", &Rect, _OSD_INDEX_YELLOW, _OSD_INDEX_BLACK);
        Delay_DelayMs(500);
    }
#endif
    Delay_DelayMs(1500);
