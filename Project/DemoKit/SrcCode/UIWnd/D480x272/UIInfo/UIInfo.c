...
void SysSetFixedFlag(void)
{
    ...
    SysSetFlag(FL_LANGUAGE,                LANG_SC);
    SysSetFlag(FL_BEEP,                    BEEP_ON);
    SysSetFlag(FL_MOVIE_GSENSOR,           GSENSOR_HIGH);
...
{    
    ...
    // Movie
    #if(_MODEL_DSC_==_MODEL_DUAL_D333_)
    SysSetFlag(FL_CAR_NUM,	                    CAR_NUM_OFF);
    #endif

    if (UI_GetData(FL_MOVIE_DUAL_REC))//vincent@20150905-3
    {
...
//    Ux_SendEvent(&CustomMovieObjCtrl,NVTEVT_EXE_EV,             1,      SysGetFlag(FL_MOVIE_EV));
    ...
    #if(_MODEL_DSC_==_MODEL_DUAL_D333_)//vincent@20150911-2
