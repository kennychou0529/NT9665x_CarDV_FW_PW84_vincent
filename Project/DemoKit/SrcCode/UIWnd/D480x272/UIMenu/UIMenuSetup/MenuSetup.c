...
TMDEF_BEGIN_ITEMS(SETUP)
	...
	&&(_MODEL_DSC_!=_MODEL_DUAL_V18_LYSZ_)&&(_MODEL_DSC_!=_MODEL_DUAL_89M3_)&&(_MODEL_DSC_!=_MODEL_DUAL_D333_))    
...
int Setup_MenuCallback(UINT32 uiMessage, UINT32 uiParam)
{
    ...
    if (uiMessage == TMM_CONFIRM_OPTION)
    {
        ...
        case IDM_AUTO_POWER_OFF:
            #if(_MODEL_DSC_==_MODEL_DUAL_D333_)//vincent@20150911-2 //vincent@20150911-4
            SysSetFlag(FL_AUTO_POWER_OFF,uwOption);
            if ((SysGetFlag(FL_MOVIE_MOTION_DET) == MOVIE_MOTIONDET_OFF) && (PRIMARY_MODE_PLAYBACK != System_GetState(SYS_STATE_CURRMODE))) 
            {
        ...
        case IDM_SENSOR_ROTATE:
            ...
            break;
        case IDM_LCDOFF:
            #if(_MODEL_DSC_==_MODEL_DUAL_D333_)//vincent@20150911-5
