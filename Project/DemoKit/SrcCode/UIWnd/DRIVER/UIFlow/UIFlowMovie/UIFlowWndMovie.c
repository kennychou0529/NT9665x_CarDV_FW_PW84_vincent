...
#include "Gsensor.h"
#include "Rtc.h"//vincent@20150811-1
...
CTRL_LIST_ITEM(UIFlowWndMovie_StaticTXT_Speed)//vincent@20150811-1
CTRL_LIST_ITEM(UIFlowWndMovie_Status_CustomerLogo)
CTRL_LIST_END
...
INT32 UIFlowWndMovie_RecModeTrig(VControl *, UINT32, UINT32 *);//vincent@20150805-1
INT32 UIFlowWndMovie_LcdOnOff(VControl *, UINT32, UINT32 *);//vincent@20150916-3
...
EVENT_ITEM(NVTEVT_RECORDING_MODE_TRIG, UIFlowWndMovie_RecModeTrig)//vincent@20150805-1
EVENT_ITEM(NVTEVT_LCD_ONOFF_EVENT, UIFlowWndMovie_LcdOnOff)//vincent@20150916-3
...
#define UPDATE_ANIMATIONPOWERON "A:\\z1a_1.jpg"
...
#else
     filehdl = FileSys_OpenFile(UPDATE_ANIMATIONPOWERON,FST_OPEN_READ);
     if (filehdl!=NULL)
     {
        FileSys_CloseFile(filehdl);
        ret = TRUE;
...
//vincent@20150806-1 begin
static CHAR  Speed_buf[20];
UINT32 uiSpeed = 0;
UINT32 uiSpeedKm = 0;
void FlowMovie_IconDrawSpeed()
{
    if(uiSpeedKm <= 0.000000000001 )
    {
        sprintf(Speed_buf,"0 KM/H");
    }
    else
    {
        sprintf(Speed_buf,"%d KM/H",uiSpeedKm);        
    }
            ...
            if (MediaRec_GetStatus() == MEDIAREC_STATUS_OPENED_NOT_RECORD)
            {
                ...
                if (SysGetFlag(FL_MOVIE_CYCLIC_REC) == MOVIE_CYCLICREC_OFF)
                {
                    ...
                    if (UI_GetData(FL_IsUseFileDB))
                    {
                       ...
                       {
                          //CheckStorageErr = FlowMovie_IsStorageErr(TRUE);//vincent@20150925-1
                       } else {
                          //CheckStorageErr = FlowMovie_IsStorageErr(FALSE);//vincent@20150925-1
                       }
                    }
                    #else
...
*/
        ...
        case MOV_ST_REC|MOV_ST_ZOOM: // Isiah, implement YUV merge mode of recording func.
            #if(_MODEL_DSC_ == _MODEL_CARDV_K12_)//vincent@20150902-2
            if ((FlowMovie_GetRecCurrTime() >= 0) || FlowMovie_RecGetYUVMergeMode()  
            #else
...
//#NT#2015/01/21#KS Hung -end
    ...
    //#if(_MODEL_DSC_==_MODEL_CARDV_K12_)//vincent@20150811-1
...
INT32 UIFlowWndMovie_OnClose(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    case MOV_ST_REC|MOV_ST_ZOOM:
        ...
        //vincent@20150917-6 begin
        MediaRec_CancelCrash();
        UIFlowWndMovie_SetGsensorTrigFlag(FALSE);  
        FlowMovie_UpdateIcons(TRUE);
        bGsensorTrig=FALSE;
        //vincent@20150917-6 end
...
INT32 UIFlowWndMovie_OnKeyMenu(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    ||(_MODEL_DSC_ == _MODEL_CARDV_GT200_)  \
    ||(_MODEL_DSC_ == _MODEL_DUAL_V18_LYSZ_))	 
        case MOV_ST_REC:
        case MOV_ST_REC|MOV_ST_ZOOM:
    ...
    ||(_MODEL_DSC_ == _MODEL_DUAL_QIZHENG_M1_)\
    ||(_MODEL_DSC_ == _MODEL_DUAL_SAIBOSHI_)\
    ||(_MODEL_DSC_ == _MODEL_CARDV_K12_))//vincent@20150813-1
        case MOV_ST_REC:
        case MOV_ST_REC|MOV_ST_ZOOM:
         ...
         if(paramNum > 0)
           {
                if(paramArray[0] == NVTRET_ENTER_MENU)
                {
                    /* Create Menu window */
                    gMovData.State = MOV_ST_MENU;
                    Input_SetKeyMask(KEY_RELEASE, FLGKEY_KEY_MASK_NULL);//vincent@20150917-2
                    TM_SetMenu(&gMovieMenu);
...
INT32 UIFlowWndMovie_OnKeyMode(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    switch(gMovData.State)
    {
      case MOV_ST_VIEW:
      case MOV_ST_VIEW|MOV_ST_ZOOM:
       #if (_MODEL_DSC_ != _MODEL_CARDV_K12_) //vincent@20150813-1
        // mask key while changing primary mode
        Ux_FlushEventByRange(NVTEVT_KEY_EVT_START,NVTEVT_KEY_EVT_END);
        Input_SetKeyMask(KEY_PRESS, FLGKEY_KEY_MASK_NULL);
        Input_SetKeyMask(KEY_RELEASE, FLGKEY_KEY_MASK_NULL);
        ...
        Ux_SendEvent(&UISetupObjCtrl,NVTEVT_EXE_CHANGEDSCMODE,1,DSCMODE_CHGTO_NEXT);
         ...
         //vincent@20150813-1 begin
         #if (_MODEL_DSC_ == _MODEL_CARDV_K12_)
            uiKeyAct = paramArray[0]; 
            uiSelect=  UI_GetData(FL_MOVIE_PARKING);
            switch (uiKeyAct)
            {
                case NVTEVT_KEY_PRESS:
                    // mask key while changing primary mode
                ...
                case NVTEVT_KEY_CONTINUE:
                    ...
                    {
                        Ux_SendEvent(&CustomMovieObjCtrl, NVTEVT_EXE_MOVIEPARKING, 1, MOVIE_PARKING_ON);
                        
                    }
                    else
                    {
                        Ux_SendEvent(&CustomMovieObjCtrl, NVTEVT_EXE_MOVIEPARKING, 1, MOVIE_PARKING_OFF);
                    }
                    FlowMovie_UpdateIcons(TRUE);
                    break;
    ...
    ||(_MODEL_DSC_ == _MODEL_DUAL_V18_LYSZ_))
//do nothing
#elif (_MODEL_DSC_ == _MODEL_CARDV_K12_)//vincent@20150813-1
    uiKeyAct = paramArray[0]; 
    uiSelect=  UI_GetData(FL_MOVIE_AUDIO);
    debug_msg("UIFlowwwndMovie Key Down..%d\r\n",uiSelect);
  ...
  UINT32 uiRecSecond, uiCyclicRecTime;
    ...
    case MOV_ST_REC|MOV_ST_ZOOM:
	  ...
	  if(Gsensor_GetCrashMode()==TRUE)
	  {
		...
		{
		#if(_MODEL_DSC_ == _MODEL_CARDV_K12_)
            if(GxUSB_GetConnectType()== USB_CONNECT_NONE)//vincent@20150817-1
            {
		#endif
    			g_bgsensor = TRUE;     
    		    MediaRec_SetCrash();	
      ...
      if((uiRecSecond==10)&&(bGsensorTrig==TRUE))//10 sensonds
#else      
...
{
    debug_msg("vincent@20150817 UIFlowWndMovie_GsensorTrig before\r\n");
     if(g_bgsensor==FALSE)
     {
        debug_msg("vincent@20150817 UIFlowWndMovie_GsensorTrig after\r\n");
	g_bgsensor = TRUE;     
       MediaRec_SetCrash();	
	FlowMovie_IconDrawLockFile();
     }	 	
...
INT32 UIFlowWndMovie_RecModeTrig(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    {
        debug_msg("vincent start lapse recording  1****************************************\r\n");
        SysSetFlag(FL_MOVIE_TIMELAPSE_REC, MOVIE_TIMELAPSEREC_500MS);
        Ux_SendEvent(pCtrl, NVTEVT_KEY_SHUTTER2, 1, NVTEVT_KEY_PRESS);//stop rec
      
        //start lapse recording
        debug_msg("vincent start lapse recording  2****************************************\r\n");
        Ux_SendEvent(pCtrl, NVTEVT_KEY_SHUTTER2, 1, NVTEVT_KEY_PRESS);//start rec
...
//vincent@20150916-3 begin
INT32 UIFlowWndMovie_LcdOnOff(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    if (paramArray[0] == 0)
    {
...
EVENT_BEGIN(UIFlowWndMovie_StaticTXT_Speed)//vincent@20150811-1
EVENT_END

//----------------------UIFlowWndMovie_Status_CustomerLogoCtrl Event---------------------------
EVENT_BEGIN(UIFlowWndMovie_Status_CustomerLogo)
EVENT_END
