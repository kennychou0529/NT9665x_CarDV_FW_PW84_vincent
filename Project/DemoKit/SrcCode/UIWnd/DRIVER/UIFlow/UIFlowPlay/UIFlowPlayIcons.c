...
static void FlowPB_IconDrawDCFFileID(BOOL bShow)
{
        ...
        uiPBFileFmt & PBFMT_MP4)
    {
        ...
        if (uiPBFileFmt & PBFMT_READONLY)
         {
            //vincent@20150828-2 begin
            if ((gchaFullName[rootFolderLength+10] == 'R')\ 
                || (gchaFullName[rootFolderLength+11] == 'O'))//the case readonly(lock) file is in RO folder
            {
                strncpy(item1_Buf, (gchaFullName+rootFolderLength+10+3),20);
            }
            else//the case readonly(lock) file is in normal folder
            {
                strncpy(item1_Buf, (gchaFullName+rootFolderLength+10),20);  
            }
             //vincent@20150828-2 end
            item1_Buf[20] = '\0';
         } else {
            strncpy(item1_Buf, (gchaFullName+rootFolderLength+10),20);
            item1_Buf[20] = '\0';
...
void FlowPB_IconDrawMovBwd(BOOL bShow)
{
    if (bShow==TRUE)
    {
        #if (_MODEL_DSC_ == _MODEL_CARDV_K12_)//vincent@20150901-2
